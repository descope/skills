...
When the Docs MCP is available, use `docs_ask_question` to confirm the exact method name, option shape, and return type before writing any SDK call. Do not write a method name and add a hedge like "verify the exact name" — just verify it.
