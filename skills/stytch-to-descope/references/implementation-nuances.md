...
**Stytch uses scrypt for password hashing (verified gotcha).** When hashes arrive from Stytch support, expect scrypt. Confirm Descope's [user format JSON](https://docs.descope.com/migrate/custom/user-format-json) import path supports the hash format Stytch provides before committing to password carryover — Descope's documented import formats (bcrypt, Argon2, PBKDF2, etc.) do not include scrypt by default.

**Search API rate limit (verified gotcha).** Stytch Search endpoints are limited to **100 requests/minute**. For large user, organization, or member datasets, paginate with deliberate delays between batches or the export will throttle mid-run.
...
- **Full migration:** Import the Stytch export into Descope using the [Create User API](https://docs.descope.com/api/management/users/create-user) or [Batch Create User API](https://docs.descope.com/api/management/users/batch-create-users). See the [user format JSON guide](https://docs.descope.com/migrate/custom/user-format-json) for the expected shape. For password carryover, wait for Stytch's scrypt hash export via support before importing — do not import placeholder or missing hashes. If hashes aren't available or scrypt import isn't viable, plan a password reset on first login or use the JIT approach below.
